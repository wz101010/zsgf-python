...
# **user_followers**
> UserFollowersResultApiResponse user_followers(app_key, tag=tag, status=status, skip=skip, take=take)
...
# **user_following**
> UserFollowingResultApiResponse user_following(app_key, tag=tag, status=status, skip=skip, take=take)
...
> UserFriendsNearByResultApiResponse user_friends_near_by(x, y, distance, app_key, gender=gender, age_s=age_s, age_e=age_e, tag=tag, type=type, skip=skip, take=take)
...
with ZSGF.Client.ApiClient(configuration) as api_client:
    ...
    age_s = 56 # int | 年龄段起始 (optional)
    age_e = 56 # int | 年龄段结束 (optional)
    tag = 'tag_example' # str | 兴趣标签 (optional)

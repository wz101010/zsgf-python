...
class UnionIDSignUpRequest(BaseModel):
    """
    UnionIDSignUpRequest
    """ # noqa: E501
    user_name: Optional[Annotated[str, Field(min_length=6, strict=True, max_length=32)]] = Field(default=None, description="登录账号，可空，只能包含字母、数字、下划线和连字符", alias="userName")
    union_id: Annotated[str, Field(min_length=1, strict=True, max_length=99)] = Field(description="UnionID，长度不能超过99个字符，只能包含字母、数字、下划线和连字符", alias="unionID")
    platform: Annotated[str, Field(min_length=1, strict=True, max_length=20)] = Field(description="平台标识，长度必须在1到20个字符之间，只能包含字母和数字")
    pwd: Annotated[str, Field(min_length=6, strict=True, max_length=32)] = Field(description="密码，长度必须在6到32个字符之间，只允许英文大小写、数字、标点符号")
    nick_name: Optional[Annotated[str, Field(min_length=4, strict=True, max_length=32)]] = Field(default=None, description="用户昵称", alias="nickName")
    ...
    __properties: ClassVar[List[str]] = ["userName", "unionID", "platform", "pwd", "nickName", "avatar", "data", "email", "emailCode", "phone", "phoneCode"]

    @field_validator('union_id')
    def union_id_validate_regular_expression(cls, value):

...
class UnionIDSignUpRequest(BaseModel):
    ...
    """ # noqa: E501
    user_name: Optional[StrictStr] = Field(default=None, description="登录账号，可空", alias="userName")
    union_id: Annotated[str, Field(min_length=1, strict=True, max_length=99)] = Field(description="UnionID", alias="unionID")
    platform: Annotated[str, Field(min_length=1, strict=True, max_length=99)] = Field(description="平台标识")
    pwd: Annotated[str, Field(min_length=1, strict=True)] = Field(description="密码，6~32位")
    ...
    def pwd_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^[a-zA-Z0-9_-~!@#$%^&*]{6,32}$", value):
            raise ValueError(r"must validate the regular expression /^[a-zA-Z0-9_-~!@#$%^&*]{6,32}$/")
        return value
